    ...
    <div className="bg-white min-h-screen flex items-center justify-center px-4 py-8">
      <div className="max-w-lg w-full">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center shadow-sm">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">{t('confirmed.title')}</h1>
          <p className="text-slate-500 text-sm mb-6">
            {t('confirmed.thanks')}
          </p>

          {order && (
            <div className="bg-slate-50 rounded-xl p-4 text-start mb-6 border border-gray-200">
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-500">{t('confirmed.orderId')}</span>
              ...
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="text-slate-900">{formatDateTime(order.created_at)}</span>
              </div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-500">{t('confirmed.payment')}</span>
                <span className="text-slate-900">{order.payment_method === 'stripe' ? t('checkout.stripe') : order.payment_method === 'paypal' ? t('checkout.paypal') : t('checkout.cod')}</span>
              </div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-500">{t('confirmed.items')}</span>
                <span className="text-slate-900">{order.order_items?.length ?? 0}</span>
              </div>
              <div className="flex justify-between text-sm pt-2 border-t border-gray-200">
                <span className="font-bold text-slate-900">{t('confirmed.total')}</span>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Link
              to="/search"
              className="flex-1 h-11 flex items-center justify-center gap-2 border border-gray-300 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-colors"
            >
