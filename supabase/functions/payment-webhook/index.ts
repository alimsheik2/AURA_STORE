...
Deno.serve(async req => {
  ...
  try {
    if (provider === 'stripe') {
      ...
      await admin.rpc('apply_verified_payment_event', {
        p_provider: 'stripe',
        p_provider_event_id: event.id,
        p_event_type: event.type,
        p_provider_payment_id: obj?.id || obj?.payment_intent || null,
        p_order_id: orderId,
        p_new_status: status,
        p_payload: event,
      });
    ...
    if (provider === 'paypal') {
      ...
      if (!transmissionId || !timestamp || !sig || !certUrl) {
        return json({ error: 'Missing PayPal webhook verification headers' }, 400);
      }
      const event = JSON.parse(raw);
      const orderId = event.resource?.custom_id || event.resource?.invoice_id;
      const eventType = event.event_type || 'PAYPAL_EVENT';
      if (!orderId) return json({ ok: true, ignored: true });
      const status = (eventType === 'PAYMENT.CAPTURE.COMPLETED' || eventType === 'CHECKOUT.ORDER.APPROVED') ? 'paid' : eventType === 'PAYMENT.CAPTURE.DENIED' ? 'failed' : null;
      if (!status) return json({ ok: true, ignored: true });
      await admin.rpc('apply_verified_payment_event', {
        p_provider: 'paypal',
        p_provider_event_id: transmissionId || event.id,
        p_event_type: eventType,
        p_provider_payment_id: event.resource?.id || null,
        p_order_id: orderId,
        p_new_status: status,
        p_payload: event,
      });
    ...
    return json({ error: e instanceof Error ? e.message : 'Webhook verification failed' }, 400);
  }
});
